...
<c:if test="${fn:contains(sessionInfo.resourceList, '/sell/edit')}">
	<script type="text/javascript">
		$.canEdit = true;
	</script>
</c:if>
<c:if test="${fn:contains(sessionInfo.resourceList, '/sell/delete')}">
	<script type="text/javascript">
		$.canDelete = true;
	</script>
</c:if>
<c:if test="${fn:contains(sessionInfo.resourceList, '/sell/cancel')}">
	<script type="text/javascript">
		$.canCancel = true;
	</script>
</c:if>
<title>oil sell manage</title>
	<script type="text/javascript">
	var dataGrid;
	$(function() {
		dataGrid = $('#dataGrid').datagrid({
			url : '${ctx}' + '/sell/dataGrid',
			striped : true,
			title: 'Sell List',
			rownumbers : true,
			pagination : true,
	...
	//查询数据
	function show(){
	    $('#dataGrid').datagrid('load',serializeObject($('#sellSearchForm')));
	}
	
	function clean(){
		$('#dataGrid').datagrid('load',{});
	    $('#sellSearchForm').find('input').val('');
	}
	...
	function addFun() {
		parent.$.modalDialog({
			...
			width : 600,
			height : 300,
			href : '${ctx}/sell/addPage',
			buttons : [ {
				text : 'add',
				handler : function() {
					parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个treeGrid，所以先预定义好
					var f = parent.$.modalDialog.handler.find('#sellAddForm');
					f.submit();
				}
			} ]
		});
	...
	function cancelFun(id) {
		...
		parent.$.messager.confirm('ask', 'Are you sure to cancel the transaction？', function(b) {
			if (b) {
					progressLoad();
					$.post('${ctx}/sell/cancel', {
						id : id
					}, function(result) {
						if (result.success) {
							parent.$.messager.alert('prompt', result.msg, 'info');
					...
					$.post('${ctx}/sell/canEdit', {
						...
						if (result.success) {
							parent.$.modalDialog({
								...
								width : 600,
								height : 300,
								href : '${ctx}/sell/editPage?id=' + id,
								buttons : [ {
									text : 'edit',
									handler : function() {
										parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
										var f = parent.$.modalDialog.handler.find('#sellEditForm');
										f.submit();
									}
								} ]
							});
...
<div class="easyui-layout" data-options="fit:true,border:false">
	<div data-options="region:'north',border:false,title:'filter'" style="height:100px;overflow:hidden">
		<form id="sellSearchForm" method="post">
				<table class="grid">
					<tr>
						<th>Start Date:</th>
						<td>
					...
					<c:if test="${fn:contains(sessionInfo.resourceList, '/sell/add')}">
